...
const AddQuestionForm = (id) => {
    ...
    const submit = () => {
        try {
            const booleans = answers.map(e => e.correct)
            const emptyAnswers = answers.map(e => answers.value)

            if (answers.length === 0) {
                alert("you must add an answer")
            } else if (question.length === 0) {
                alert("Question cant be empty")
            } else if (!booleans.includes(true)) {
                alert("No correct answers marked")
            } else if (emptyAnswers.includes("")) {
